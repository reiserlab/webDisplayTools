...
checkThrows('encodeSetAnalogCal deadband > 2000 rejected', () =>
    Wire.encodeSetAnalogCal(1, 'setDeadband', 2001)
);
checkThrows('encodeSetAnalogCal deadband needs an integer', () =>
    Wire.encodeSetAnalogCal(1, 'setDeadband')
);
{
    ...
    check(
        'decodeAnalogCal header',
        String([rec.version, rec.adcBits, rec.source, rec.sdMirrorOk]),
        '1,12,eeprom,true'
    );
    check(
        'decodeAnalogCal ch1 points',
        String([
            rec.channels[0].valid,
            rec.channels[0].rawOpen,
            rec.channels[0].rawGnd,
            rec.channels[0].deadbandMv
        ]),
        'true,3700,100,20'
    );
    check('decodeAnalogCal ch1 line a', Math.round(rec.channels[0].gainMvPerCount * 1000), 2778); // 10000/3600
    check('decodeAnalogCal ch1 line b', Math.round(rec.channels[0].offsetMv), -278);
    check(
        'decodeAnalogCal ch2 invalid → no line',
        String([rec.channels[1].valid, rec.channels[1].gainMvPerCount, rec.channels[1].deadbandMv]),
        'false,,50'
    );
    check(
...
checkBool(
    'capability bit 6 is ai_cal',
    Wire.decodeControllerInfo(Uint8Array.from([0x04, 0x00, 0xc2, 1, 0x63])).capabilities.join(','),
    'g6_mode,v2_local_storage,io_ext,ai_cal'
);
