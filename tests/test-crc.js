...
function check(name, got, expected) {
    ...
    const gotStr = typeof got === 'number' ? '0x' + got.toString(16).toUpperCase().padStart(expected > 0xff ? 4 : 2, '0') : got;
    const expStr = typeof expected === 'number' ? '0x' + expected.toString(16).toUpperCase().padStart(expected > 0xff ? 4 : 2, '0') : expected;
...
const v2L = new Uint8Array(53); v2L[0] = 0x01; v2L[1] = 0x10;
check('2L Oneshot all-zero (53B)', G6CRC.crc8Autosar(v2L), 0xC6);
const v16L = new Uint8Array(203); v16L[0] = 0x01; v16L[1] = 0x30;
check('16L Oneshot all-zero (203B)', G6CRC.crc8Autosar(v16L), 0x6D);

console.log('\n=== CRC-16 protocol vectors ===');
check('Frame-header all-zero (FR + 0x00 0x00)', G6CRC.crc16CcittFalse(new Uint8Array([0x46, 0x52, 0x00, 0x00])), 0xFD6B);
...
checkBool('reference JSON has crc8_autosar.vectors', !!(crcRef && crcRef.crc8_autosar && crcRef.crc8_autosar.vectors));
checkBool('reference JSON has crc16_ccitt_false.vectors', !!(crcRef && crcRef.crc16_ccitt_false && crcRef.crc16_ccitt_false.vectors));
...
const rows = 2, cols = 10, pixelRows = rows * 20, pixelCols = cols * 20, N = 8;
const frames = [];
for (let f = 0; f < N; f++) {
...
const buf = PatEncoder.encode({
    generation: 'G6', gs_val: 1, numFrames: N, rowCount: rows, colCount: cols,
    pixelRows, pixelCols, frames, stretchValues: new Array(N).fill(1),
    arena_id: 1, observer_id: 0,
});
...
if (parsed) {
    ...
    let mm = 0;
    for (let f = 0; f < N; f++) for (let i = 0; i < frames[f].length; i++) if (frames[f][i] !== parsed.frames[f][i]) mm++;
...
mute();
try { PatParser.parsePatFile(bad.buffer, { strict: true }); } catch (e) { strictHeaderThrew = e.message.includes('header CRC-8 mismatch'); }
unmute();
...
mute();
try { PatParser.parsePatFile(bad2.buffer, { strict: true }); } catch (e) { strictFrameThrew = e.message.includes('CRC-16 mismatch'); }
unmute();
...
mute();
try { const r = PatParser.parsePatFile(bad.buffer, { strict: false }); tolerantOK = r && r.numFrames === N; } catch (_) {}
unmute();
