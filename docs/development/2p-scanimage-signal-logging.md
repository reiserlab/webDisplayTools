...
## 0. Minimal version (recommended for the first run) — everything inside the TIFF

Nothing here triggers ScanImage; you start the Grab by hand. Aux triggers only stamp times into frames
...
4. Ch2 / Ch3 as before. That is all you need to test "pedestal and 300 Hz line vs duty" and to compare
   the two firmwares.
5. **Optional, only if a photodiode is at the rig:** photodiode → rear SMB **AI3 = Channel 4** (< 2 Vpp),
   Channels window → Save. It is the only way to tell a residual at low duty apart from SiPM recovery.

Not needed for this test: recording the line clock (it is generated by the same FPGA as the pixel clock,
so image columns already are the line-clock phase, and its shape is known from the scope), the frame
clock, the AO marker, or the Data Recorder. §§ 1–4 below describe those for the full version.
