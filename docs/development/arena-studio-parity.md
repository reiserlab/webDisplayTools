...
| Runtime variables (Studio v0.84) — the Run view panel for `runtime_controls:` (bounded operator changes, explicit Apply, next-trial activation, logged provenance; `js/runtime-controls.js` + `js/studio-runtime-vars.js`) and the `requires:` capability gate that refuses to run protocols needing `flow_control`. The parser preserves both keys on round trip, so a protocol authored with them still opens/exports in the standalone — it just has no panel and no gate there | ✗ | ✗ (preserved, not shown; no run gate) |

| Run-log replay (Studio v0.88, ported from Arena Studio Alt into Classic): Run-view replay bar + "Recent runs" chooser (course repo `runlogs/<bench>/index.json`), protocol found from the log's run_metadata (sha-matched, incl. git history), log-only fallback, `?repo=…&replay=…` links, synchronized 3D window with the cartoon fly + ball holder. Pure core in `js/studio-replay.js` (dual-export, `tests/test-studio-replay.js`); hardware-output interlock for the whole replay. Alt keeps its own reference replay (`js/arena-studio-alt.js`, gated to `#alt`) | ✗ | ✗ |
