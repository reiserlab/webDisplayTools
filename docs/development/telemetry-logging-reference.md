...
| `heading_tare` (`type:"heading_tare"`, `dir:"bridge"`) | `bridge.py` `Pipeline.handle_line` (bridge ≥ 3.2) | first FicTrac frame after a `bias`-carrying `config` — the epoch's heading zero | `ms` (relative), `hd0_deg`. Offline reconstruction of `idx` from `hd` MUST subtract the most recent `hd0_deg` (wrapped to (−180,180]) or every index is off by `round(hd0/gain)` — see `docs/development/closed-loop-bias.md` |
