...
| `idx` | int | derived | frame index the bridge computed: bridge ≥ 3.3 `round((coupling·Δheading + offset + b(t))/deg_per_frame) mod frames` with Δheading the UNWRAPPED turn since the epoch's `heading_tare` (≤ 3.2: `round((wrap180(hd−hd0) + offset + b)/gain)`); `coupling`/`deg_per_frame`(alias `gain`)/`offset`/`frames` come from the browser's `{type:"config"}` message (pushed per startClosedLoop by the runner); `0` when the pitch is unset | `bridge.py` `frame_index_from_heading` |
...
| `runner` with `phase:"led-activation"` | `js/arena-runner-g6.js` `_drainLed` (Studio v0.59; graded since v0.86) | every LED write during a Mode-3 trial with `led_activation` (one per ≥ 4 mV change of the zone level, after the AO ack) | `on` (level > 0), `index` (the displayed frame the level came from), `ledPercent` (the level that took effect — a ramp is a staircase of these), `mv` (commanded AO, v0.86). The trial's normalized spec (`baseline`, `zones`, sugar echo, `warning`) rides the `trial-running` event as `ledActivation` |
| `log` | `js/arena-runner-g6.js:1454` | runner INFO/WARN lines | `message`, `level` |
| `config` (`type:"config"`, no `event`) | `js/fictrac-bridge-client.js` per trial | closed-loop trial start | `fictrac_port`, `gain`, `offset`, `frames`, **`bias`** `{type, amplitude, frequency}` (bridge ≥ 3.2; `startClosedLoop` always carries one, `{type:"none"}` when unauthored, and `stopClosedLoop` pushes `none`) — the bridge's `idx` parameters |
| `bias_config` (`type:"bias_config"`, `dir:"bridge"`, no `event`/`rx_ms`) | `bridge.py` `Pipeline.set_bias` (bridge ≥ 3.2) | every `config` carrying `bias` — the **phase-clock zero** of the waveform | `ms` (**same relative timebase as the behaviour rows' `ms`**), `bias {type ∈ none·constant·sine·square, amplitude (deg/s peak), frequency (Hz)}`. `b(t)` is analytic (`bias_angle_deg`), so this event + a row's `ms` reconstructs the bias angle exactly; `idx` in every row already has it folded in |
| `heading_tare` (`type:"heading_tare"`, `dir:"bridge"`) | `bridge.py` `Pipeline.advance_heading` (bridge ≥ 3.2; `reason` since 3.3) | first FicTrac frame after a `config` carrying `bias` or `epoch:true` — the epoch's heading zero — or after a FicTrac restart | `ms` (relative), `hd0_deg`, `reason` ∈ `epoch` · `ft_reset` (3.3). Offline reconstruction of `idx` from `hd` MUST start the unwrapped turn at the tare row (≤ 3.2: subtract `hd0_deg`, wrapped) or every index is off by `round(hd0/deg_per_frame)` — see `docs/development/closed-loop-bias.md` |
