...
| B | **Injected stall** (checks the flagging path; **2026-09-15 result: cannot be injected on the lab card** — arm 3 confirmed in force for 44 k legacy-seek reads on card sn 000014ca with 0 stalls, where the bench card stalled every ≈ 24.5 k; the flagging path itself was demonstrated on rig03 (6 flags) and on the bench drills, so B counts as "arm verified, card does not reproduce"): in the browser console `await Studio.setSdDiag(3)`, open `protocols/mode3_drill_1trial.yaml` and run it as a 1-iteration Soak (File ▾ → Soak…, iterations 1), then `await Studio.setSdDiag(0)` | during the run: `display gap NN ms (sd_slow, body) in trial …` lines; at the end `⚠ stimulus quality: … flagged`; after `setSdDiag(0)`, Console identity shows `sd diag 0` | at least one trial flagged; the soak did NOT stop; switches back to 0 |
